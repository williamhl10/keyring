#key ring generation using the special XOR after swapping
...
class KeyGen():
	...
	def __init__(self):
			...
			#perform swapping for maximum security and then XOR further
			...
			for i in range(0, len(self.AES)):
				...
				if self.AES[i][0] % 2 == 1 and self.AES[(i+2)%num_chunks][0] % 2 == 1: #make sure vals from PRF
					...
					if self.AES[i][0] == joint_member_test:
						...
						#XOR further for regeneration
						#print self.AES[(i+2)%num_chunks][1], hold1

						#print self.AES[(i+2)%num_chunks + 1]
						#print self.HMAC[(i+2)%num_chunks + 1]

						self.AES[(i+2)%num_chunks + 1][1] = self.AES[(i+2)%num_chunks + 1][1] ^ hold2[1]
						self.HMAC[(i+2)%num_chunks + 1][1] = self.HMAC[(i+2)%num_chunks + 1][1] ^ hold1[1]
...
#print key.AES[0][1] ^ key.AES[2][1] ^ key.HMAC[2][1]
